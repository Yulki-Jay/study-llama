...
def main():
    ...
    print('*'*50)
    print('模型开始训练')
    trainer = pl.Trainer(devices=4,max_epochs=5,accelerator='gpu',default_root_dir='./checkpoints',limit_train_batches=5,limit_val_batches=1,limit_test_batches=5,strategy='ddp_find_unused_parameters_true') 
    # 因为有些参数没有用到，所以会出现报错的情况，因此如果采用ddp的话，就需要设置strategy='ddp_find_unused_parameters_true'
    # trainer = pl.Trainer(devices=4,max_epochs=5,accelerator='gpu',num_nodes=1,default_root_dir='./checkpoints',strategy='ddp_find_unused_parameters_true')
